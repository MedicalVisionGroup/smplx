...
def draw_curve(
    ...
    y_error_list: tp.Optional[tp.List[float]] = None,
    color: str = "red",
...
def draw_list_of_list_of_boxplots(
    data_list: tp.List[tp.List[tp.List[float]]],
    x_ticks_list: tp.List[str],
    grp_name_list: tp.List[str],
    ...
    # draw groups of (T, N) boxplots.
    # T steps, N groups

    len_T = len(data_list)
    len_N = len(data_list[0])
    assert all([len(data) == len_N for data in data_list])

    group_pos = np.arange(1, len_T + 1)
    offsets = np.linspace(-box_w * (len_N - 1) / 2, box_w * (len_N - 1) / 2, len_N)
    offsets *= 1 + box_gap

    for n in range(len_N):
        data = [data_list[t][n] for t in range(len_T)]
        this_pos = group_pos + offsets[n]
        ...
        ax.boxplot(
            ...
            widths=box_w,
            patch_artist=True,
            boxprops=dict(facecolor=color_list[n]),
            manage_ticks=True,
            showfliers=True,
            medianprops=medianprops,
            flierprops=flierprops,
    ...
    if grp_name_list is not None and show_legend:
        legend_handles = [
            Patch(
                facecolor=color_list[n], edgecolor=color_list[n], label=grp_name_list[n]
            )
            for n in range(len_N)
        ]
