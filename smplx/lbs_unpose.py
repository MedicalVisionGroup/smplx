...
from .lbs import (batch_rigid_transform, batch_rodrigues, blend_shapes,
                  vertices2joints)
...
def invert_rigid_transform_with_keypoints(
    ...
    if kpt_parents is None:
        ...
        assert torch.allclose(
            idx_nn[0], idx_nn, atol=1e-6, rtol=1e-6
...
def lbs_unpose(
    ...
    assert torch.allclose(
        J_inverted, J, atol=2e-6, rtol=1e-6
    ...
    assert torch.allclose(
        verts_invert, v_posed, atol=0.005, rtol=1e-4
    ), f"max abs diff: {torch.max(torch.abs(verts_invert - v_posed))}"
